 ...
 * root. */
static inline void MVM_gc_write_barrier(struct MVMThreadContext *tc, MVMCollectable *update_root, MVMCollectable *referenced) {
...
static inline void MVM_gc_write_barrier_no_update_referenced(struct MVMThreadContext *tc, MVMCollectable *update_root, MVMCollectable *referenced) {
    if (((update_root->flags2 & MVM_CF_SECOND_GEN) && referenced && !(referenced->flags2 & MVM_CF_SECOND_GEN)))
