...
static uint32_t utf8_encode(uint8_t *bp, MVMCodepoint cp) {
...
MVMString * MVM_string_utf8_c8_decode(struct MVMThreadContext *tc, const MVMObject *result_type,
    ...
    while (state.cur_byte < bytes) {
        uint8_t decode_byte = utf8[state.cur_byte];
...
uint32_t MVM_string_utf8_c8_decodestream(struct MVMThreadContext *tc, MVMDecodeStream *ds,
    ...
    while (cur_bytes && !reached_stopper) {
        ...
        state.result_pos = 0;
        state.utf8 = (const uint8_t*)cur_bytes->bytes;
        ...
        while (state.cur_byte < bytes) {
            /* Process a byte. */
            uint8_t decode_byte = state.utf8[state.cur_byte];
...
char * MVM_string_utf8_c8_encode_substr(struct MVMThreadContext *tc,
    ...
    if (replacement)
        repl_bytes = (uint8_t *) MVM_string_utf8_c8_encode_substr(tc, replacement, &repl_length, 0, -1, NULL);
