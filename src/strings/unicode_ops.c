...
typedef struct ring_buffer ring_buffer;
    ...
    static void print_sub_node (sub_node subnode) {
        int64_t min    = next_node_min(subnode);
        char * min_sign = min < 0 ? "-" : "";
        int64_t max    = next_node_max(subnode);
        char * max_sign = max < 0 ? "-" : "";
        max = max < 0 ? -max : max;
        min = min < 0 ? -min : min;
        fprintf(stderr,
...
static int32_t NFD_and_push_collation_values (MVMThreadContext *tc, MVMCodepoint cp, collation_stack *stack, MVMCodepointIter *ci, char *name) {
    ...
    int32_t result_size = initial_collation_norm_buf_size;
    int64_t rtrn        = 0;
...
static void collation_push_MVM_values (MVMThreadContext *tc, MVMCodepoint cp, collation_stack *stack, MVMCodepointIter *ci, char *name) {
    ...
    if (is_Block_Tangut(cp) || MVM_coll_key.s.primary <= 0 || MVM_coll_key.s.secondary <= 0 || MVM_coll_key.s.tertiary <= 0) {
        ...
        /* Unified_Ideograph=True */
            ...
            int32_t NFD_rtrn = NFD_and_push_collation_values(tc, cp, stack, ci, name);
            if (NFD_rtrn) {
                return;
            }
            else {
...
static int64_t collation_push_cp (MVMThreadContext *tc, collation_stack *stack, MVMCodepointIter *ci, int *cp_maybe, int cp_num, char *name) {
    ...
    if (query != -1) {
        ...
        /* Otherwise we need to check the next codepoint(s) (0 < sub_node_elems) */
            ...
            int64_t last_good_i      =  0,
                     last_good_result = -1;
            int64_t i, result = query;
            DEBUG_PRINT_SUB_NODE(main_nodes[query]);
...
static void ring_buffer_done(MVMThreadContext *tc, ring_buffer *buffer) {
    ...
    /* Otherwise we need to copy it manually */
        ...
        int32_t out_location     = buffer->codes_out_count - 1;
        int32_t buf_location     = buffer->location;
...
int64_t MVM_unicode_string_compare(MVMThreadContext *tc, MVMString *a, MVMString *b,
    ...
    int64_t compare_by_cp_rtrn = 0;
    int64_t pos_a = 0, pos_b = 0, rtrn = 0;
    int16_t grab_a_done = 0, grab_b_done = 0;
    /* From 0 to 2 for primary, secondary, tertiary levels */
    int16_t   level_a = 0,   level_b = 0;
    ...
    if (!result) {
        ...
        for (i = 0; i < prefixes_len; i++) {
            ...
            if (!strncmp(cname, prefixes[i], str_len)) {
                char *reject = NULL;
                int64_t rtrn = strtol(cname + strlen(prefixes[i]), &reject, 16);
...
MVMString * MVM_unicode_get_name(MVMThreadContext *tc, int64_t codepoint) {
    ...
    /* Look up name. */
        ...
        int32_t codepoint_row = MVM_codepoint_to_row_index(tc, codepoint);
        if (codepoint_row != -1) {
...
uint32_t MVM_unicode_get_case_change(MVMThreadContext *tc, MVMCodepoint codepoint, int32_t case_,
    ...
    if (case_ == MVM_unicode_case_change_type_fold) {
        ...
        if (folding_index) {
            int32_t is_simple = MVM_unicode_get_property_int(tc,
        ...
        int32_t special_casing_index = MVM_unicode_get_property_int(tc,
        ...
        if (special_casing_index) {
            ...
            int32_t changes_index = MVM_unicode_get_property_int(tc,
...
int64_t MVM_unicode_name_to_property_value_code(MVMThreadContext *tc, int64_t property_code, MVMString *name) {
    if (property_code <= 0 || MVM_NUM_PROPERTY_CODES <= property_code)
        ...
        uint64_t cname_length;
        char *cname = MVM_string_ascii_encode(tc, name, &cname_length, 0);
        int32_t code = unicode_cname_to_property_value_code(tc, property_code, cname, cname_length);
        MVM_free(cname);
        return code;
    }
}
...
MVMCodepoint MVM_unicode_find_primary_composite(MVMThreadContext *tc, MVMCodepoint l, MVMCodepoint c) {
    ...
    const int32_t *pcs  = comp_p[plane][upper][lower];
    if (pcs) {
        int32_t entries = pcs[0];
        int32_t i;
