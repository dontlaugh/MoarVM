...
static struct MVMStrHashTableControl *maybe_grow_hash(struct MVMThreadContext *tc,
    ...
    if (control->cur_items < max_items
        ...
        uint8_t *metadata = MVM_str_hash_metadata(control);
    ...
    uint8_t *entry_raw_orig = MVM_str_hash_entries(control);
    uint8_t *metadata_orig = MVM_str_hash_metadata(control);
    uint8_t entry_size = control->entry_size;
...
static uint64_t hash_fsck_internal(struct MVMThreadContext *tc, struct MVMStrHashTableControl *control, uint32_t mode) {
    ...
    uint8_t *entry_raw = MVM_str_hash_entries(control);
    uint8_t *metadata = MVM_str_hash_metadata(control);
    uint32_t bucket = 0;
        ...
        if (!*metadata) {
            ...
            if (problem) {
                ...
                if (display == 2 || (display == 1 && error_count)) {
                    ...
                    uint64_t len = MVM_string_graphs(tc, key);
