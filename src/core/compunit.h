...
MVM_STATIC_INLINE MVMString * MVM_cu_string(MVMThreadContext *tc, MVMCompUnit *cu, uint32_t idx) {
    MVMString *s = cu->body.strings[idx];
    return s ? s : MVM_cu_obtain_string(tc, cu, idx);
}

MVM_STATIC_INLINE void MVM_cu_ensure_string_decoded(MVMThreadContext *tc, MVMCompUnit *cu, uint32_t idx) {
