...
typedef struct {
    ...
    uint32_t active_top;
    uint32_t active[MVM_JIT_ARCH_NUM_REG];

    /* Values still left to do (heap) */
    MVM_VECTOR_DECL(int32_t, worklist);
    /* Retired values (to be assigned registers) (heap) */
    MVM_VECTOR_DECL(int32_t, retired);
    /* Spilled values */
    MVM_VECTOR_DECL(int32_t, spilled);
...
static inline uint32_t order_nr(uint32_t tile_idx) {
    return tile_idx * 2;
}


static inline int32_t is_definition(ValueRef *v) {
    return (v->value_idx == 0);
}

static inline int32_t is_arglist_ref(MVMJitTileList *list, ValueRef *v) {
    return (list->items[v->tile_idx]->op == MVM_JIT_ARGLIST);
}


static inline int32_t live_range_is_empty(LiveRange *range) {
...
static int32_t live_range_init(RegisterAllocator *alc) {
    int32_t idx = alc->values_num++;
...
static void live_range_add_ref(RegisterAllocator *alc, LiveRange *range, uint32_t tile_idx, uint32_t value_idx) {
...
static struct Hole * live_range_has_hole(LiveRange *value, uint32_t order_nr) {
    struct Hole *h;
...
static UnionFind * value_set_find(UnionFind *sets, int32_t key) {
...
static int32_t value_set_union(UnionFind *sets, LiveRange *values, int32_t a, int32_t b) {
...
static inline void heap_swap(int32_t *heap, int32_t a, int32_t b) {
    int32_t t = heap[a];
    heap[a]    = heap[b];
    heap[b]    = t;
}
...
static void live_range_heap_down(LiveRange *values, int32_t *heap, int32_t top, int32_t item,
                                 int32_t (*cmp)(LiveRange *values, int32_t a, int32_t b)) {
    while (item < top) {
        int32_t left = item * 2 + 1;
...
static void live_range_heap_up(LiveRange *values, int32_t *heap, int32_t item,
                               int32_t (*cmp)(LiveRange* values, int32_t a, int32_t b)) {
    while (item > 0) {
        int32_t parent = (item-1)/2;
...
static int32_t live_range_heap_pop(LiveRange *values, int32_t *heap, size_t *top,
                                    int32_t (*cmp)(LiveRange* values, int32_t a, int32_t b)) {
    int32_t v = heap[0];
    int32_t t = --(*top);
...
static void live_range_heap_push(LiveRange *values, int32_t *heap, size_t *top, int32_t v,
                                 int32_t (*cmp)(LiveRange* values, int32_t a, int32_t b)) {
...
static uint32_t live_range_heap_peek(LiveRange *values, int32_t *heap) {
    return values[heap[0]].start;
}

static void live_range_heapify(LiveRange *values, int32_t *heap, int32_t top,
                               int32_t (*cmp)(LiveRange* values, int32_t a, int32_t b)) {
    int32_t i = top, mid = top/2;
    while (i-- > mid) {
...
static int32_t values_cmp_first_ref(LiveRange *values, int32_t a, int32_t b) {
    return values[a].start - values[b].start;
}

static int32_t values_cmp_last_ref(LiveRange *values, int32_t a, int32_t b) {
    return values[a].end - values[b].end;
}

static inline MVMJitTile * first_tile(MVMJitTileList *list, LiveRange *value) {
 ...
 * allocator is ported. */
static int8_t try_preferred_register(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTile *tile, MVMBitmap reg_perm) {
...
static int8_t allocate_register(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list, LiveRange *value) {
...
static void free_register(struct MVMThreadContext *tc, RegisterAllocator *alc, uint8_t reg_num) {
    assert(!MVM_bitmap_get_low(alc->reg_free, reg_num));
    MVM_bitmap_set_low(&alc->reg_free, reg_num);
}

static void assign_register(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list,
...
static inline void close_hole(RegisterAllocator *alc, int32_t ref, uint32_t tile_idx) {
...
static inline void open_hole(RegisterAllocator *alc, int32_t ref, uint32_t tile_idx) {
    LiveRange *v = alc->values + ref;
...
static void find_holes(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list) {
    int32_t i, j;
    uint32_t k;

    MVMJitExprTree *tree = list->tree;
    uint32_t bitmap_size = (alc->values_num >> 5) + 1;
...
#define _SUCC(_a, _z) (list->blocks[(_a)].succ[(_z)])
    ...
    for (j = list->blocks_num - 1; j >= 0; j--) {
        ...
        int32_t start = list->blocks[j].start, end = list->blocks[j].end;
        if (list->blocks[j].num_succ == 2) {
            ...
            for (k = 0; k < bitmap_size; k++) {
                ...
                while (additions) {
                    int32_t bit = MVM_FFS(additions) - 1;
                    int32_t val = (k << 6) + bit;
        ...
        for (i = end - 1; i >= start; i--) {
            ...
            if (tile->op == MVM_JIT_ARGLIST) {
                ...
                uint32_t nchild = MVM_JIT_EXPR_NCHILD(tree, tile->node);
                int32_t *args  = MVM_JIT_EXPR_LINKS(tree, tile->node);
                for (k = 0; k < nchild; k++) {
                    int32_t ref  = value_set_find(alc->sets, MVM_JIT_EXPR_LINKS(tree, args[k])[0])->idx;
...
static void determine_live_ranges(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list) {
    uint32_t i, j;
    int32_t num_phi = 0; /* pessimistic but correct upper bound of number of holes */
    MVMJitExprTree *tree = list->tree;
    ...
    for (i = 0; i < list->items_num; i++) {
        ...
        if (tile->op == MVM_JIT_COPY) {
            int32_t ref        = MVM_JIT_EXPR_LINKS(tree, node)[0];
            ...
            int32_t last_child = MVM_JIT_EXPR_FIRST_CHILD(tree, node) + MVM_JIT_EXPR_NCHILD(tree, node) - 1;
            int32_t ref        = tree->nodes[last_child];
            ...
            uint32_t num_args = MVM_JIT_EXPR_NCHILD(tree, node);
            int32_t *refs = MVM_JIT_EXPR_LINKS(tree, node);
            _DEBUG("Adding %d references to ARGLIST node", num_args);
            for (j = 0; j < num_args; j++) {
                int32_t value = MVM_JIT_EXPR_LINKS(tree, refs[j])[0];
                int32_t idx   = value_set_find(alc->sets, value)->idx;
...
static void active_set_add(struct MVMThreadContext *tc, RegisterAllocator *alc, int32_t a) {
    ...
    for (i = 0; i < alc->active_top; i++) {
        int32_t b = alc->active[i];
        if (alc->values[b].end > alc->values[a].end) {
            /* insert a before b */
            memmove(alc->active + i + 1, alc->active + i, sizeof(int32_t)*(alc->active_top - i));
            alc->active[i] = a;
            alc->active_top++;
            return;
        }
...
static void active_set_expire(struct MVMThreadContext *tc, RegisterAllocator *alc, uint32_t order_nr) {
    uint32_t i;
    for (i = 0; i < alc->active_top; i++) {
...
static int32_t earliest_active_value(struct MVMThreadContext *tc, RegisterAllocator *alc, uint32_t tile_order_nr) {
...
static void active_set_splice(struct MVMThreadContext *tc, RegisterAllocator *alc, uint32_t to_splice) {
    int32_t i ;
    ...
    if (i >= 0 && (uint32_t)i < alc->active_top) {
        /* shift out */
...
static void spill_set_release(struct MVMThreadContext *tc, RegisterAllocator *alc, uint32_t order_nr) {
    while (alc->spilled_num > 0 && alc->values[alc->spilled[0]].end <= order_nr) {
        int32_t spilled = live_range_heap_pop(alc->values, alc->spilled, &alc->spilled_num,
                                               values_cmp_last_ref);
...
static int32_t insert_load_before_use(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list,
                                       ValueRef *ref, int32_t load_pos) {
    int32_t n = live_range_init(alc);
...
static int32_t insert_store_after_definition(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list,
                                              ValueRef *ref, int32_t store_pos) {
    int32_t n       = live_range_init(alc);
...
static uint32_t select_live_range_for_spill(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list, int32_t code_pos, MVMBitmap reg_perm) {
    int i;
...
static void live_range_spill(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list,
...
static void prepare_arglist_and_call(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list,
                                     int32_t arglist_idx, int32_t call_idx) {
    MVMJitTile *arglist_tile = list->items[arglist_idx],
                  *call_tile = list->items[call_idx];
    MVMJitExprTree *tree = list->tree;
    uint32_t num_args = MVM_JIT_EXPR_NCHILD(tree, arglist_tile->node);
    int32_t *args    = MVM_JIT_EXPR_LINKS(tree, arglist_tile->node);
    int32_t           arg_values[16];
    ...
    for (i = 0, j = 0; i < alc->active_top; i++) {
        ...
        uint32_t code_pos = order_nr(call_idx);
        if (v->end > code_pos && live_range_has_hole(v, code_pos) == NULL) {
            /* surviving values need to be spilled */
            int32_t spill_pos = MVM_jit_spill_memory_select(tc, alc->compiler, v->reg_type);
...
static inline void process_tile(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list, uint32_t tile_cursor) {
    MVMJitTile *tile = list->items[tile_cursor];

    if (tile->op == MVM_JIT_ARGLIST) {
        uint32_t call_idx = tile_cursor + 1;
        ...
        uint32_t arglist_idx = tile_cursor - 1;
        uint32_t call_idx    = tile_cursor;
...
static void process_live_range(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list, int32_t v) {
    int8_t reg;
    ...
    if ((MVM_JIT_RESERVED_REGISTERS & reg_perm) != 0) {
        ...
        while ((reg = allocate_register(tc, alc, list, alc->values + v)) < 0) {
            ...
            uint32_t to_spill   = select_live_range_for_spill(tc, alc, list, tile_order_nr, reg_perm);
            uint32_t spill_pos  = MVM_jit_spill_memory_select(tc, alc->compiler, alc->values[to_spill].reg_type);
...
static void linear_scan(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list) {
    ...
    while (tile_cursor < list->items_num) {
        ...
        if (alc->worklist_num > 0 && live_range_heap_peek(alc->values, alc->worklist) <= order_nr(tile_cursor)) {
            ...
            int32_t live_range = live_range_heap_pop(alc->values, alc->worklist, &alc->worklist_num, values_cmp_first_ref);
            int32_t tile_order_nr = alc->values[live_range].start;
...
static void enforce_operand_requirements(struct MVMThreadContext *tc, RegisterAllocator *alc, MVMJitTileList *list) {
