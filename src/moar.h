...
/* clang also refuses to cast as (atomic_uintptr_t)(v), but doing this works for gcc and
...
#define AO_READ(v) (v)
#define AO_CAST(v) (atomic_uintptr_t)(v)
#endif
...
#define MVM_trycas(addr, old, new) MVM_trycas_AO((volatile atomic_uintptr_t *)(addr), AO_CAST(old), AO_CAST(new))
#else
#define MVM_trycas(addr, old, new) AO_compare_and_swap_full((volatile atomic_uintptr_t *)(addr), (atomic_uintptr_t)(old), (atomic_uintptr_t)(new))
#endif
...
#define MVM_incr(addr) atomic_fetch_add((volatile atomic_uintptr_t *)(addr), 1)
#define MVM_decr(addr) atomic_fetch_sub((volatile atomic_uintptr_t *)(addr), 1)
#define MVM_add(addr, add) atomic_fetch_add((volatile atomic_uintptr_t *)(addr), (add))
...
#define MVM_casptr(addr, old, new) ((void *)MVM_cas((atomic_uintptr_t *)(addr), (uintptr_t)(old), (uintptr_t)(new)))
...
#define MVM_store(addr, new) atomic_store((volatile atomic_uintptr_t *)(addr), AO_CAST(new))
#define MVM_load(addr) atomic_load((volatile atomic_uintptr_t *)(addr))
...
atomic_uintptr_t AO_fetch_compare_and_swap_emulation(volatile atomic_uintptr_t *addr, atomic_uintptr_t old_val, atomic_uintptr_t new_val);
# define AO_fetch_compare_and_swap_full(addr, old, newval) \
    AO_fetch_compare_and_swap_emulation(addr, old, newval)
#endif

/* Returns original. Use only on atomic_uintptr_t-sized values (including pointers). */
#define MVM_incr(addr) AO_fetch_and_add1_full((volatile atomic_uintptr_t *)(addr))
#define MVM_decr(addr) AO_fetch_and_sub1_full((volatile atomic_uintptr_t *)(addr))
#define MVM_add(addr, add) AO_fetch_and_add_full((volatile atomic_uintptr_t *)(addr), (atomic_uintptr_t)(add))

/* Returns the old value dereferenced at addr. */
#define MVM_cas(addr, old, new) AO_fetch_compare_and_swap_full((addr), (old), (new))

/* Returns the old pointer value dereferenced at addr. Provided for a tiny bit of type safety. */
#define MVM_casptr(addr, old, new) ((void *)MVM_cas((atomic_uintptr_t *)(addr), (atomic_uintptr_t)(old), (atomic_uintptr_t)(new)))
...
/* Need to use these to assign to or read from any memory locations on
 * which the other atomic operation macros are used... */
#define MVM_store(addr, new) AO_store_full((volatile atomic_uintptr_t *)(addr), (atomic_uintptr_t)(new))
#define MVM_load(addr) AO_load_full((volatile atomic_uintptr_t *)(addr))
