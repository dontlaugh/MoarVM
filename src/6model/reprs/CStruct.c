...
static void get_attribute(MVMThreadContext *tc, MVMSTable *st, MVMObject *root,
    ...
    if (slot >= 0) {
        ...
        switch (kind) {
        case MVM_reg_obj: {
            int32_t type      = repr_data->attribute_locations[slot] & MVM_CSTRUCT_ATTR_MASK;
            int32_t real_slot = repr_data->attribute_locations[slot] >> MVM_CSTRUCT_ATTR_SHIFT;
