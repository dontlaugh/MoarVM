...
#define GET_UI8(pc, idx)    *((uint8_t *)((pc) + (idx)))
...
static inline uint32_t GET_N32(const uint8_t *pc, int32_t idx) {
    float retval;
    memcpy(&retval, pc + idx, sizeof(retval));
    return retval;
}
    ...
    if (g->num_deopt_addrs == g->alloc_deopt_addrs) {
        ...
        if (g->deopt_addrs)
            g->deopt_addrs = MVM_realloc(g->deopt_addrs,
                g->alloc_deopt_addrs * sizeof(int32_t) * 2);
        else
            g->deopt_addrs = MVM_malloc(g->alloc_deopt_addrs * sizeof(int32_t) * 2);
    }
}
...
int32_t MVM_spesh_graph_add_deopt_annotation(struct MVMThreadContext *tc, MVMSpeshGraph *g,
                                          MVMSpeshIns *ins_node, uint32_t deopt_target,
                                          int32_t type) {
    /* Add an annotations. */
...
static void add_logged_annotation(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshIns *ins_node,
                                  uint8_t *pc) {
    MVMSpeshAnn *ann = MVM_spesh_alloc(tc, g, sizeof(MVMSpeshAnn));
...
static void add_cached_annotation(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshIns *ins_node,
                                  uint8_t *pc) {
    MVMSpeshAnn *ann = MVM_spesh_alloc(tc, g, sizeof(MVMSpeshAnn));
...
static int32_t is_catch_handler(struct MVMThreadContext *tc, MVMSpeshGraph *g, int32_t handler_idx) {
    return g->handlers[handler_idx].category_mask & MVM_EX_CAT_CATCH;
}

/* Checks if a basic block already has a particular successor. */
static int32_t already_succs(struct MVMThreadContext *tc, MVMSpeshBB *bb, MVMSpeshBB *succ) {
    int32_t i = 0;
...
static int32_t spesh_dispatchy(uint16_t opcode) {
...
static void build_cfg(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMStaticFrame *sf,
                      int32_t *existing_deopts, int32_t num_existing_deopts,
                      int32_t *existing_deopt_synths, int32_t num_existing_deopt_synths,
                      int32_t *deopt_usage_info, MVMSpeshIns ***deopt_usage_ins_out) {
    ...
    uint32_t *byte_to_ins_flags = MVM_calloc(g->bytecode_size, sizeof(uint32_t));
    ...
    uint8_t *active_handlers = MVM_calloc(1, g->num_handlers);
    int32_t num_active_handlers = 0;
    ...
    uint8_t    *pc       = g->bytecode;
    uint8_t    *end      = g->bytecode + g->bytecode_size;
    uint32_t    ins_idx  = 0;
    uint8_t     next_bbs = 1; /* Next iteration (here, first) starts a BB. */
    uint32_t    num_osr_points = 0;

    MVMBytecodeAnnotation *ann_ptr = MVM_bytecode_resolve_annotation(tc, &sf->body, sf->body.bytecode - pc);

    for (i = 0; i < g->num_handlers; i++) {
        if (g->handlers[i].start_offset != (uint32_t)-1 && g->handlers[i].goto_offset != (uint32_t)-1) {
    ...
    while (pc < end) {
        ...
        /* The sp_resumption op is also var args. */
            ...
            MVMSpeshResumeInit *resume_init = &(g->resume_inits[((uint16_t *)args)[1]]);
        ...
        for (i = 0; i < info->num_operands; i++) {
            ...
            case MVM_operand_literal: {
                ...
                case MVM_operand_ins: {
                    ...
                    uint32_t target = GET_UI32(args, arg_size);
        ...
        if (opcode == MVM_OP_jumplist) {
            int64_t n = MVM_BC_get_I64(args, 0);
            for (i = 0; i <= n; i++)
    ...
    while (cur_bb) {
         ...
         */
        if (cur_bb == g->entry) {
            int32_t num_bbs = 1 + g->num_handlers + num_osr_points;
            ...
            for (i = 0; i < g->num_handlers; i++) {
                if (is_catch_handler(tc, g, i)) {
                    uint32_t offset = g->handlers[i].goto_offset;
        ...
        /* Otherwise, non-entry basic block. */
            ...
            switch (cur_bb->last_ins->info->opcode) {
                case MVM_OP_jumplist: {
                    ...
                    int64_t jump_bbs = cur_bb->last_ins->operands[0].lit_i64 + 1;
                    int64_t num_bbs = jump_bbs + num_active_handlers;
                ...
                case MVM_OP_goto: {
                    ...
                    int64_t num_bbs = 1 + num_active_handlers;
                    uint32_t   offset = cur_bb->last_ins->operands[0].ins_offset;
    ...
    if (existing_deopts) {
        for (i = 0; i < num_existing_deopts; i ++) {
            if (existing_deopts[2 * i + 1] >= 0) {
                ...
                uint32_t deopt = (uint32_t)existing_deopts[2 * i + 1];
                uint32_t bytecode_pos = MVM_spesh_deopt_bytecode_pos(deopt);
    ...
    if (deopt_usage_ins_out && deopt_usage_info) {
        ...
        while (1) {
            int32_t offset = deopt_usage_info[idx];
            if (offset == -1)
                break;
 ...
 * place). */
static int32_t is_handler_reg(struct MVMThreadContext *tc, MVMSpeshGraph *g, uint16_t reg) {
    uint32_t num_handlers = g->num_handlers;
    uint32_t i;
...
static void insert_object_null_instructions(struct MVMThreadContext *tc, MVMSpeshGraph *g) {
    MVMSpeshBB *insert_bb = g->entry->linear_next;
    uint16_t *local_types = g->sf->body.local_types;
    uint16_t  num_locals = g->sf->body.num_locals;
    uint16_t i;
...
static void add_predecessors(struct MVMThreadContext *tc, MVMSpeshGraph *g) {
 ...
 * the entry point. */
static void dfs(MVMSpeshBB **rpo, int32_t *insert_pos, uint8_t *seen, MVMSpeshBB *bb) {
    int32_t i;
    seen[bb->idx] = 1;
    for (i = 0; i < bb->num_succ; i++) {
...
static void iter_check(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB **rpo, int32_t *doms, int32_t iters) {
    if (iters > 100000) {
#ifdef NDEBUG
        uint32_t k;
        char *dump_msg = MVM_spesh_dump(tc, g);
...
static int32_t intersect(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB **rpo, int32_t *doms, int32_t finger1, int32_t finger2) {
    int32_t iters = 0;
...
static int32_t * compute_dominators(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB **rpo) {
    uint32_t i, j, changed;
    ...
    int32_t *doms = MVM_malloc(g->num_bbs * sizeof(int32_t));
    doms[0] = 0;
    ...
    while (changed) {
        ...
        for (i = 1; i < g->num_bbs; i++) {
            ...
            for (j = 0; j < b->num_pred; j++) {
                if (j != (uint32_t)chosen_pred) {
                    int32_t p_idx = b->pred[j]->rpo_idx;
                    if (doms[p_idx] != -1)
...
static void add_child(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB *target, MVMSpeshBB *to_add) {
...
static void add_children(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB **rpo, int32_t *doms) {
    uint32_t i;
    for (i = 0; i < g->num_bbs; i++) {
        MVMSpeshBB *bb   = rpo[i];
        uint32_t   idom = doms[i];
        if ((uint32_t)idom != i)
...
static void add_to_frontier_set(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB *target, MVMSpeshBB *to_add) {
    MVMSpeshBB **new_df;
    int32_t i;
...
static void add_dominance_frontiers(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB **rpo, int32_t *doms) {
    int32_t j;
    MVMSpeshBB *b = g->entry;
    while (b) {
        if (b->num_pred >= 2) { /* Thus it's a join point. */
            for (j = 0; j < b->num_pred; j++) {
                int32_t runner      = b->pred[j]->rpo_idx;
                int32_t finish_line = doms[b->rpo_idx];
...
static SSAVarInfo * initialize_ssa_var_info(struct MVMThreadContext *tc, MVMSpeshGraph *g) {
        ...
        while (ins) {
            for (i = 0; i < ins->info->num_operands; i++) {
                if ((ins->info->operands[i] & MVM_operand_rw_mask) == MVM_operand_write_reg) {
                    uint16_t written = ins->operands[i].reg.orig;
                    int32_t  found   = 0;
                    int32_t  j;
                    ...
                    if (!found) {
                        if (var_info[written].num_ass_nodes % 8 == 0) {
                            int32_t new_size = var_info[written].num_ass_nodes + 8;
...
static void insert_phi_functions(struct MVMThreadContext *tc, MVMSpeshGraph *g, SSAVarInfo *var_info) {
    int32_t    *has_already  = MVM_calloc(g->num_bbs, sizeof(int32_t));
    int32_t    *work         = MVM_calloc(g->num_bbs, sizeof(int32_t));
    MVMSpeshBB **worklist     = MVM_calloc(g->num_bbs, sizeof(MVMSpeshBB *));
    int32_t     worklist_top = 0;
    int32_t     iter_count   = 0;

    /* Go over all locals. */
    int32_t var, i, j, found;
...
static int32_t which_pred(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB *y, MVMSpeshBB *x) {
    int32_t i;
...
static void rename_locals(struct MVMThreadContext *tc, MVMSpeshGraph *g, SSAVarInfo *var_info, MVMSpeshBB *x) {
    int32_t i;
    ...
    while (a) {
        ...
        int32_t is_phi = a->info->opcode == MVM_SSA_PHI;
        if (!is_phi) {
            for (i = 0; i < a->info->num_operands; i++) {
                if ((a->info->operands[i] & MVM_operand_rw_mask) == MVM_operand_read_reg) {
                    uint16_t orig = a->operands[i].reg.orig;
                    int32_t  st   = var_info[orig].stack_top;
                    if (st >= 0)
        ...
        for (i = 0; i < a->info->num_operands; i++) {
            if (is_phi || (a->info->operands[i] & MVM_operand_rw_mask) == MVM_operand_write_reg) {
                uint16_t orig = a->operands[i].reg.orig;
                int32_t reg_i = var_info[orig].count;
                ...
                if (var_info[orig].stack_top + 1 >= var_info[orig].stack_alloc) {
                    ...
                    var_info[orig].stack = MVM_realloc(var_info[orig].stack,
                        var_info[orig].stack_alloc * sizeof(int32_t));
                }
    ...
    for (i = 0; i < x->num_succ; i++) {
        ...
        int32_t     j = which_pred(tc, g, y, x);
        MVMSpeshIns *p = y->first_ins;
        while (p && p->info->opcode == MVM_SSA_PHI) {
            uint16_t orig = p->operands[j + 1].reg.orig;
            int32_t  st   = var_info[orig].stack_top;
            if (st >= 0)
 ...
 * needed. */
static void ssa(struct MVMThreadContext *tc, MVMSpeshGraph *g) {
    SSAVarInfo *var_info;
    int32_t i, num_locals;

    /* Compute dominance frontiers. */
    MVMSpeshBB **rpo  = MVM_spesh_graph_reverse_postorder(tc, g);
    int32_t    *doms = compute_dominators(tc, g, rpo);
    ...
    g->fact_counts = MVM_spesh_alloc(tc, g, num_locals * sizeof(uint16_t));
    for (i = 0; i < num_locals; i++) {
...
MVMSpeshGraph * MVM_spesh_graph_create_from_cand(struct MVMThreadContext *tc, MVMStaticFrame *sf,
                                                 MVMSpeshCandidate *cand, uint32_t cfg_only,
...
void MVM_spesh_graph_mark(struct MVMThreadContext *tc, MVMSpeshGraph *g, MVMGCWorklist *worklist) {
    uint16_t i, j, num_locals, num_facts, *local_types;
    ...
    /* Mark inlines. */
    for (uint32_t k = 0; k < g->num_inlines; k++)
