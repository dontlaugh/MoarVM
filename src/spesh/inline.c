...
static int is_graph_inlineable(struct MVMThreadContext *tc, MVMSpeshGraph *inliner,
    ...
    int32_t same_hll = target_sf->body.cu->body.hll_config ==
    ...
    while (bb) {
        ...
        while (ins) {
            int32_t opcode = ins->info->opcode;
            int32_t is_phi = opcode == MVM_SSA_PHI;
...
static void add_deopt_usages(struct MVMThreadContext *tc, MVMSpeshGraph *g,
    ...
    while (deopt_usage_info[usage_idx] != -1) {
        ...
        usage_idx += 2;
        for (i = 0; i < count; i++) {
            int32_t deopt_idx = deopt_usage_info[usage_idx++];
    ...
    *effective_size = get_effective_size(tc, cand);
    ...
    if (is_graph_inlineable(tc, inliner, target_sf, runbytecode_ins, ig,
        ...
        for (i = 0; i < ig->num_inlines; i++) {
            ...
            uint16_t reg = ig->inlines[i].code_ref_reg;
            uint32_t j;
...
static void fix_wval(struct MVMThreadContext *tc, MVMSpeshGraph *inliner,
    ...
    int16_t     dep      = to_fix->operands[1].lit_i16;
    int64_t     idx      = to_fix->info->opcode == MVM_OP_wval
        ? to_fix->operands[2].lit_i16
        : to_fix->operands[2].lit_i64;
    if (dep >= 0 && (uint16_t)dep < sourcecu->body.num_scs) {
        ...
        if (sc) {
            ...
            if (MVM_sc_is_object_immediately_available(tc, sc, idx)) {
                int16_t ss  = MVM_spesh_add_spesh_slot_try_reuse(tc, inliner, (MVMCollectable *)MVM_sc_get_object(tc, sc, idx));
                to_fix->info = MVM_op_get_op(MVM_OP_sp_getspeshslot);
                to_fix->operands[1].lit_i16 = ss;
            }
            else {
                int16_t ss  = MVM_spesh_add_spesh_slot_try_reuse(tc, inliner, (MVMCollectable *)sc);
...
static MVMSpeshBB * merge_graph(struct MVMThreadContext *tc, MVMSpeshGraph *inliner,
        ...
        uint32_t *inline_boundary_handler, uint16_t bytecode_size,
        MVMCallsite *cs) {
    MVMSpeshFacts **merged_facts;
    uint16_t      *merged_fact_counts;
    ...
    int32_t same_comp_unit = inliner->sf->body.cu == inlinee->sf->body.cu;
    int32_t same_hll = same_comp_unit || inliner->sf->body.cu->body.hll_config ==
    ...
    for (i = 0; i < inliner->sf->body.num_locals; i++) {
        uint16_t vers = inliner->fact_counts[i];
        for (j = 0; j < vers; j++) {
    ...
    for (i = 0; i < MVM_VECTOR_ELEMS(inlinee->deopt_pea.materialize_info); i++) {
        ...
        if (mi_new.num_attr_regs) {
            mi_new.attr_regs = MVM_malloc(mi_new.num_attr_regs * sizeof(uint16_t));
    ...
    merged_fact_counts = MVM_spesh_alloc(tc, inliner,
        (inliner->num_locals + inlinee->num_locals) * sizeof(uint16_t));
    memcpy(merged_fact_counts, inliner->fact_counts,
        inliner->num_locals * sizeof(uint16_t));
    memcpy(merged_fact_counts + inliner->num_locals, inlinee->fact_counts,
        inlinee->num_locals * sizeof(uint16_t));
    ...
    if (inlinee->num_deopt_addrs) {
        ...
        if (inliner->deopt_addrs)
            inliner->deopt_addrs = MVM_realloc(inliner->deopt_addrs,
                inliner->alloc_deopt_addrs * sizeof(int32_t) * 2);
        else
            inliner->deopt_addrs = MVM_malloc(inliner->alloc_deopt_addrs * sizeof(int32_t) * 2);
        memcpy(inliner->deopt_addrs + inliner->num_deopt_addrs * 2,
            inlinee->deopt_addrs, inlinee->alloc_deopt_addrs * sizeof(int32_t) * 2);
    ...
    if (!inliner->local_types && inliner->num_locals) {
        int32_t local_types_size = inliner->num_locals * sizeof(uint16_t);
    ...
    if (!inliner->lexical_types && inliner->num_lexicals) {
        int32_t lexical_types_size = inliner->num_lexicals * sizeof(uint16_t);
    ...
    inliner->lexical_types = MVM_realloc(inliner->lexical_types,
        (inliner->num_lexicals + inlinee->num_lexicals) * sizeof(uint16_t));
...
static void rewrite_returns(struct MVMThreadContext *tc, MVMSpeshGraph *inliner,
    ...
    int32_t initial_last_result_version = runbytecode_ins->info->opcode != MVM_OP_sp_runbytecode_v
        ? inliner->fact_counts[runbytecode_ins->operands[0].reg.orig]
        : -1;
    int32_t saw_return = 0;
    while (bb) {
        MVMSpeshIns *ins = bb->first_ins;
        while (ins) {
            uint16_t opcode = ins->info->opcode;
        ...
        if (bb == inlinee_last_bb) {
            int32_t final_last_result_version = runbytecode_ins->info->opcode != MVM_OP_sp_runbytecode_v
                ? inliner->fact_counts[runbytecode_ins->operands[0].reg.orig]
                : -1;
        ...
        while (ins) {
            ...
            switch (opcode) {
                ...
                case MVM_OP_sp_getarg_s: {
                    uint16_t idx = ins->operands[1].lit_i16;
                    ins->info = MVM_op_get_op(MVM_OP_set);
...
void MVM_spesh_inline(struct MVMThreadContext *tc, MVMSpeshGraph *inliner,
        ...
        MVMSpeshIns *resume_init, uint16_t bytecode_size) {
